// This file was generated by SquareLine Studio
// SquareLine Studio version: SquareLine Studio 1.6.1
// LVGL version: 8.3.11
// Project name: LifePuck_Project

#include "ui.h"

lv_obj_t * uic_settingsButton;
lv_obj_t * ui_MainScreen = NULL;
lv_obj_t * ui_timer = NULL;
lv_obj_t * ui_lifeUp = NULL;
lv_obj_t * ui_lifeDown = NULL;
lv_obj_t * ui_Life = NULL;
lv_obj_t * ui_minutes = NULL;
lv_obj_t * ui_Bat = NULL;
lv_obj_t * ui_percent = NULL;
lv_obj_t * ui_settingsButton = NULL;
lv_obj_t * ui_tempLife = NULL;
// event funtions

// build funtions

void ui_MainScreen_screen_init(void)
{
    ui_MainScreen = lv_obj_create(NULL);
    lv_obj_clear_flag(ui_MainScreen, LV_OBJ_FLAG_SCROLLABLE);      /// Flags
    ui_object_set_themeable_style_property(ui_MainScreen, LV_PART_MAIN | LV_STATE_DEFAULT, LV_STYLE_BG_COLOR,
                                           _ui_theme_color_Purple);
    ui_object_set_themeable_style_property(ui_MainScreen, LV_PART_MAIN | LV_STATE_DEFAULT, LV_STYLE_BG_OPA,
                                           _ui_theme_alpha_Purple);

    ui_timer = lv_btn_create(ui_MainScreen);
    lv_obj_set_width(ui_timer, 200);
    lv_obj_set_height(ui_timer, 50);
    lv_obj_set_x(ui_timer, 0);
    lv_obj_set_y(ui_timer, -190);
    lv_obj_set_align(ui_timer, LV_ALIGN_CENTER);
    ui_object_set_themeable_style_property(ui_timer, LV_PART_MAIN | LV_STATE_DEFAULT, LV_STYLE_BG_COLOR,
                                           _ui_theme_color_Purple);
    ui_object_set_themeable_style_property(ui_timer, LV_PART_MAIN | LV_STATE_DEFAULT, LV_STYLE_BG_OPA,
                                           _ui_theme_alpha_Purple);
    lv_obj_set_style_bg_img_opa(ui_timer, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_img_recolor(ui_timer, lv_color_hex(0xFAFAFA), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_img_recolor_opa(ui_timer, 255, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_lifeUp = lv_btn_create(ui_MainScreen);
    lv_obj_set_width(ui_lifeUp, 337);
    lv_obj_set_height(ui_lifeUp, 194);
    lv_obj_set_x(ui_lifeUp, 0);
    lv_obj_set_y(ui_lifeUp, -60);
    lv_obj_set_align(ui_lifeUp, LV_ALIGN_CENTER);
    ui_object_set_themeable_style_property(ui_lifeUp, LV_PART_MAIN | LV_STATE_DEFAULT, LV_STYLE_BG_COLOR,
                                           _ui_theme_color_Purple);
    ui_object_set_themeable_style_property(ui_lifeUp, LV_PART_MAIN | LV_STATE_DEFAULT, LV_STYLE_BG_OPA,
                                           _ui_theme_alpha_Purple);
    ui_object_set_themeable_style_property(ui_lifeUp, LV_PART_MAIN | LV_STATE_PRESSED, LV_STYLE_BG_COLOR,
                                           _ui_theme_color_Red);
    ui_object_set_themeable_style_property(ui_lifeUp, LV_PART_MAIN | LV_STATE_PRESSED, LV_STYLE_BG_OPA,
                                           _ui_theme_alpha_Red);

    ui_lifeDown = lv_btn_create(ui_MainScreen);
    lv_obj_set_width(ui_lifeDown, 337);
    lv_obj_set_height(ui_lifeDown, 170);
    lv_obj_set_x(ui_lifeDown, 0);
    lv_obj_set_y(ui_lifeDown, 125);
    lv_obj_set_align(ui_lifeDown, LV_ALIGN_CENTER);
    ui_object_set_themeable_style_property(ui_lifeDown, LV_PART_MAIN | LV_STATE_DEFAULT, LV_STYLE_BG_COLOR,
                                           _ui_theme_color_Purple);
    ui_object_set_themeable_style_property(ui_lifeDown, LV_PART_MAIN | LV_STATE_DEFAULT, LV_STYLE_BG_OPA,
                                           _ui_theme_alpha_Purple);
    ui_object_set_themeable_style_property(ui_lifeDown, LV_PART_MAIN | LV_STATE_PRESSED, LV_STYLE_BG_COLOR,
                                           _ui_theme_color_Green);
    ui_object_set_themeable_style_property(ui_lifeDown, LV_PART_MAIN | LV_STATE_PRESSED, LV_STYLE_BG_OPA,
                                           _ui_theme_alpha_Green);

    ui_Life = lv_label_create(ui_MainScreen);
    lv_obj_set_width(ui_Life, LV_SIZE_CONTENT);   /// 1
    lv_obj_set_height(ui_Life, LV_SIZE_CONTENT);    /// 1
    lv_obj_set_x(ui_Life, 0);
    lv_obj_set_y(ui_Life, 56);
    lv_obj_set_align(ui_Life, LV_ALIGN_CENTER);
    lv_label_set_text(ui_Life, "0");
    lv_obj_set_style_text_color(ui_Life, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(ui_Life, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(ui_Life, &ui_font_qq99, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_minutes = lv_label_create(ui_MainScreen);
    lv_obj_set_width(ui_minutes, LV_SIZE_CONTENT);   /// 0
    lv_obj_set_height(ui_minutes, LV_SIZE_CONTENT);    /// 40
    lv_obj_set_x(ui_minutes, 0);
    lv_obj_set_y(ui_minutes, -185);
    lv_obj_set_align(ui_minutes, LV_ALIGN_CENTER);
    lv_label_set_text(ui_minutes, "55:00");
    lv_label_set_recolor(ui_minutes, "true");
    lv_obj_set_style_text_font(ui_minutes, &ui_font_mid, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_Bat = lv_label_create(ui_MainScreen);
    lv_obj_set_width(ui_Bat, LV_SIZE_CONTENT);   /// 1
    lv_obj_set_height(ui_Bat, LV_SIZE_CONTENT);    /// 1
    lv_obj_set_x(ui_Bat, 72);
    lv_obj_set_y(ui_Bat, 206);
    lv_obj_set_align(ui_Bat, LV_ALIGN_CENTER);
    lv_label_set_text(ui_Bat, "BAT:");
    lv_obj_set_style_text_color(ui_Bat, lv_color_hex(0x706F6F), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(ui_Bat, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(ui_Bat, &lv_font_montserrat_16, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_percent = lv_label_create(ui_MainScreen);
    lv_obj_set_width(ui_percent, LV_SIZE_CONTENT);   /// 1
    lv_obj_set_height(ui_percent, LV_SIZE_CONTENT);    /// 1
    lv_obj_set_x(ui_percent, 278);
    lv_obj_set_y(ui_percent, 411);
    lv_label_set_text(ui_percent, "100%");
    lv_obj_set_style_text_color(ui_percent, lv_color_hex(0x9C9595), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(ui_percent, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(ui_percent, &lv_font_montserrat_26, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_settingsButton = lv_imgbtn_create(ui_MainScreen);
    lv_imgbtn_set_src(ui_settingsButton, LV_IMGBTN_STATE_RELEASED, NULL, &ui_img_1791433664, NULL);
    lv_imgbtn_set_src(ui_settingsButton, LV_IMGBTN_STATE_PRESSED, NULL, &ui_img_1791433664, NULL);
    lv_obj_set_width(ui_settingsButton, 80);
    lv_obj_set_height(ui_settingsButton, 64);
    lv_obj_set_x(ui_settingsButton, -142);
    lv_obj_set_y(ui_settingsButton, -190);
    lv_obj_set_align(ui_settingsButton, LV_ALIGN_CENTER);

    ui_tempLife = lv_label_create(ui_MainScreen);
    lv_obj_set_width(ui_tempLife, LV_SIZE_CONTENT);   /// 1
    lv_obj_set_height(ui_tempLife, LV_SIZE_CONTENT);    /// 1
    lv_obj_set_x(ui_tempLife, -145);
    lv_obj_set_y(ui_tempLife, -100);
    lv_obj_set_align(ui_tempLife, LV_ALIGN_CENTER);
    lv_label_set_text(ui_tempLife, "");
    lv_obj_set_style_text_font(ui_tempLife, &lv_font_montserrat_30, LV_PART_MAIN | LV_STATE_DEFAULT);

    uic_settingsButton = ui_settingsButton;

}

void ui_MainScreen_screen_destroy(void)
{
    if(ui_MainScreen) lv_obj_del(ui_MainScreen);

    // NULL screen variables
    ui_MainScreen = NULL;
    ui_timer = NULL;
    ui_lifeUp = NULL;
    ui_lifeDown = NULL;
    ui_Life = NULL;
    ui_minutes = NULL;
    ui_Bat = NULL;
    ui_percent = NULL;
    uic_settingsButton = NULL;
    ui_settingsButton = NULL;
    ui_tempLife = NULL;

}
